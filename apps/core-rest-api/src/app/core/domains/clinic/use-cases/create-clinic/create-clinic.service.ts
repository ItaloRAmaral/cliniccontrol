...
import { CreateClinicInputDto } from './create-clinic-dto';

export class CreateClinicService {
  constructor(private clinicDatabaseRepository: ClinicDatabaseRepository) {}

  async execute(createClinicDto: ICreateClinicServiceProps): Promise<ClinicEntity> {
    // Validate
    ...
    // Create
    const createClinic =
      await this.clinicDatabaseRepository.createClinic(createClinicDto);
